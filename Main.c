...
{
int n, r_size;
char buf[BUF_SIZE];/* BUF _SIZE : constante définie dans file.h */
while(size > 0)
{
...
if(n == 0) break;// S'il n'y a plus rien à lire, on break la boucle.
...
char * my_readlink(const char *lnk)// Lit la valeur d'un lien dur
{
...
int trouver_fichier(const char *file, int argc, char **argv)
{
int i = -1;
while(argc-- > 0) {
i++;
...
int fichiers_existe(const char *file, int argc, char **argv)
{
if(argc <= 0) return 1;
return trouver_fichier(file, argc, argv);
}
...
int extraire_fichier(int tfd, int argc, char **argv)
{
file_struct fs;
int read_val;
char * file_buf;
...
return 0;
if(!fichiers_existe(fs.path, argc, argv))// Si les fichiers n'existent pas
return 1;
...
else fd = STDIN_FILENO; //si archive non précisé, alors on lit sur la console
while(extraire_fichier(fd, argc, argv));
if(fd > 2) close(fd);
}
...
else fd = STDIN_FILENO;// Si pas d'archive précisé, on lit sur la console
...
int fd;
if(tar_path) {
if(tar_cmd == TAR_UPDATE || tar_cmd == TAR_APPEND) {// Si la commande est ajouter ou modifier
...
else fd = STDOUT_FILENO; // sinon on l'écrit sur le canal sortant
while(argc-- > 0) {
